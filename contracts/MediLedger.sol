...
{
    event allPatient(string indexed fid,string name,address contractAddress,address paddress);
    event allDoctor(string indexed did,string name,address contractAddress,address daddress);
    ...
    function addPatient(string memory _name,string memory _fid,string memory walletid) public 
    {
    ...
    function addDoctor(string memory name,string memory did,string memory walletid) public 
    {
        require(msg.sender==admin);
        address wid=address(bytes20(bytes(walletid)));
...
contract Patient is allDoctors
{
    ...
    constructor(string memory _name,string memory _fid,address _accounts,address _admin,address payable  _walletid)
    {
        name=_name;
        pid=_fid;
        accounts=_accounts;
    ...
    function passBill() payable public 
    {
    ...
    function isDoctor(address docadd) view public returns(bool) 
    {
    ...
    function getPendingBills() view public returns(string[] memory)
    {
        return pendingBills;
    }

    function getPassedBills() view public returns(string[] memory)
    {
        return passedBills;
    }

    function getPendingRefund() view  public returns(uint)
    {
